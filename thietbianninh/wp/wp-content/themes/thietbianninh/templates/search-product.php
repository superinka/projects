	...
	<div class="content_list_product">
		...
		<?php
			...
			<!--Pagination-->
						...
						<div class="pagination nav-pag pull-right">
							<ul class="list-inline">
								...
								<?php else: ?>
									<li class="disabled prev"><a><?php _e('<i class="fa fa-caret-left"></i>', 'yatheme'); ?></a></li>
								<?php endif; ?>
			...
			if ($paged > 3 && $product_query->max_num_pages > 4) {?>
	      	...
	      	for ($i = 1; $i<= $max ; $i++){?>
								...
								<?php else: ?>
								<li class="disabled pagination"><a><?php _e('<i class="fa fa-caret-right"></i>', 'yatheme'); ?>
								</a></li>
								<?php endif; ?>
							</ul>
						</div>
		...
		?>
			<div class="alert alert-warning alert-dismissible" role="alert">
				...
				<p><?php _e('No product found.', 'yatheme'); ?></p>
			</div>
		<?php
			}
		?>
